...
export function RankingTable() {
  ...
  return <section className="panel overflow-hidden">
    <div className="flex flex-col gap-4 border-b border-white/[.07] p-5 lg:flex-row lg:items-center lg:justify-between">
      <div><h2 className="text-base font-semibold">Top Ranking</h2><p className="mt-1 text-xs text-muted-foreground">Liquid, non-pegged, non-meme assets between $30M and $100M market cap</p></div>
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm">
      ...
      <tbody className="divide-y divide-white/[.055]">{rows.map((asset) => <tr key={asset.coinId} className="group transition-colors hover:bg-white/[.025]"><td className="number px-5 py-3.5 text-xs text-muted-foreground">{asset.rank.toString().padStart(2, "0")}</td><td><div className="flex items-center gap-2.5"><div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-slate-700 to-slate-900 text-[10px] font-bold text-slate-100">{asset.symbol.slice(0, 2)}</div><div><p className="font-medium">{asset.symbol}<span className="ml-1.5 text-xs font-normal text-muted-foreground">{asset.name}</span></p></div></div></td><td className="number font-medium">{asset.price === null ? "—" : `$${asset.price.toLocaleString("en-US", { maximumFractionDigits: asset.price < 1 ? 4 : 2 })}`}</td><td className="number text-muted-foreground">{asset.marketCap === null ? "—" : `$${compactCurrency(asset.marketCap)}`}</td><td className="number text-muted-foreground">—</td><td className="number text-muted-foreground">{asset.volume === null ? "—" : `$${compactCurrency(asset.volume)}`}</td><td><div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted"><div className={cn("h-full rounded-full", asset.score > 75 ? "bg-emerald-400" : asset.score > 55 ? "bg-amber-400" : "bg-rose-400")} style={{ width: `${asset.score}%` }} /></div><span className="number font-medium">{asset.score.toFixed(1)}</span></div></td><td><SignalBadge signal={asset.signal} /></td><td className="px-5"><Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100"><Star className="h-3.5 w-3.5" /></Button></td></tr>)}{rows.length === 0 && <tr><td colSpan={9} className="p-10 text-center text-sm text-muted-foreground">{emptyMessage}</td></tr>}</tbody>
    </table></div>
