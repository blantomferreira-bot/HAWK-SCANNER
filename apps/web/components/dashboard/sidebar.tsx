...
const navigation: Array<[LucideIcon, string]> = [
  [LayoutDashboard, "Overview"], [Gauge, "Scanner"], [BarChart3, "Analytics"], [Star, "Watchlists"], [BellRing, "Alerts"],
];
  ...
  return <>
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-white/[0.07] bg-[#090e19] transition-transform lg:static lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
      ...
      <nav className="flex-1 space-y-1 p-3">
        ...
        {navigation.map(([Icon, label], index) => <button key={label} className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors", index === 0 ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-white/[.045] hover:text-foreground")}><Icon className="h-4 w-4" />{label}{index === 4 && <span className="ml-auto rounded-full bg-rose-500 px-1.5 text-[10px] font-bold text-white">3</span>}</button>)}
        <p className="px-3 pb-2 pt-7 text-[10px] font-semibold uppercase tracking-[.16em] text-muted-foreground">Control</p>
        <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-white/[.045] hover:text-foreground"><ShieldCheck className="h-4 w-4" />Data integrity</button>
        <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-white/[.045] hover:text-foreground"><Settings className="h-4 w-4" />Settings</button>
      </nav>
